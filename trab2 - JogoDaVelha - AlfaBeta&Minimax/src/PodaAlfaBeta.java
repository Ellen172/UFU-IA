...
public class PodaAlfaBeta {
    ...
    public Estado max(Estado estado, int alfa, int beta){
        ...
        for (Estado filho : filhos) {
            Estado estadoMin = min(filho, alfa, beta);
            if(estadoMin.getFator() > best){
                best = estadoMin.getFator();
                estadoFinal = estadoMin;
            }
            if(estadoMin.getFator() >= beta) return estadoMin;
            if(estadoMin.getFator() > alfa) alfa = estadoMin.getFator();
        }
        return estadoFinal;
    }

    public Estado min(Estado estado, int alfa, int beta){
        //verifica estado terminal
        char terminal = estadoTerminal(estado);
        if(terminal == 'X') estado.setFator(1);
        if(terminal == 'O') estado.setFator(-1);
        ...
        Estado estadoFinal = null;
        for (Estado filho : filhos) {
            Estado estadoMax = max(filho, alfa, beta);
            if(estadoMax.getFator() < best){
                best = estadoMax.getFator();
                estadoFinal = estadoMax;
            }
            if(estadoMax.getFator() <= alfa) return estadoMax;
            if(estadoMax.getFator() < beta) beta = estadoMax.getFator();
        }
        return estadoFinal;
    }
